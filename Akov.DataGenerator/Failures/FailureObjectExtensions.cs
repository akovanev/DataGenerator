...
namespace Akov.DataGenerator.Failures
{
    internal static class FailureObjectExtensions
    {
        internal static List<FailureObject> ToFailureObjectList(this Failure? failure)
        {
            ...
            double formatProbability = failure?.Format ?? 0;
            double rangeProbability = failure?.Range ?? 0;

            double[] group = {nullProbability, formatProbability, rangeProbability};
            ...
            return new List<FailureObject>
            {
                ...
                new FailureObject(FailureType.Nullable, new Range(sums[0], sums[1])),
                new FailureObject(FailureType.Format, new Range(sums[1], sums[2])),
