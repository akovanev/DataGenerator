...
internal class FailureObject(FailureType failureType, Range randomRange)
{
        ...
        double? outOfRangeProbability)
    {
        ...
        {
            new(FailureType.None, new Range(0, sums[0])),
            new(FailureType.Custom, new Range(sums[0], sums[1])),
            new(FailureType.Nullable, new Range(sums[1], sums[2])),
            new(FailureType.Range, new Range(sums[2], sums[3]))
        };
    }
}